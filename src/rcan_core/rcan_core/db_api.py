...
_FETCH_NEXT_READY_PANEL = (
    ...
    "CALL {\n"
    "    WITH start\n"
    "    MATCH path = (start)-[rels:NEXT*0..]->(candidate:Panel)\n"
    "    WHERE coalesce(candidate.state,'') <> 'Installed'\n"
    "      AND CASE\n"
    "            WHEN size(rels) = 0 THEN true\n"
    "            ELSE all(idx IN range(0, size(rels) - 1) WHERE\n"
    "                CASE\n"
    "                    WHEN rels[idx].order IS NOT NULL THEN toInteger(rels[idx].order) = idx + 1\n"
    "                    WHEN rels[idx].sequence IS NOT NULL THEN toInteger(rels[idx].sequence) = idx + 1\n"
    "                    WHEN rels[idx].name IS NOT NULL THEN toInteger(last(split(rels[idx].name, ' '))) = idx + 1\n"
    "                    ELSE false\n"
    "                END\n"
    "            )\n"
    "          END\n"
