...
void URRobotiqGripper_NODE(){
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "in the node function");
    URRobotiqGripperNode = rclcpp::Node::make_shared("URRobotiqGripper_SC_node");
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "after creating node");
}
...
void GripperCloseURRobotiq(){
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "in the close function");
    auto GRIPPER_SC = URRobotiqGripperNode->create_client<ros2_robotiqgripper::srv::RobotiqGripper>("/Robotiq_Gripper");
    auto req_msg = std::make_shared<ros2_robotiqgripper::srv::RobotiqGripper::Request>();
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "after crating client");
    // GripperCLose:
    req_msg->action = "CLOSE";
    auto REQUEST = GRIPPER_SC->async_send_request(req_msg);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "before spin");
    rclcpp::spin_some(URRobotiqGripperNode);
    //rclcpp::spin_some(URRobotiqGripperNode);
    //rclcpp::spin_some(URRobotiqGripperNode);
    //rclcpp::spin_some(URRobotiqGripperNode);
    //rclcpp::spin_some(URRobotiqGripperNode);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "after spin");
        ...
        for (ros2srrc_data::msg::Action STEP : SEQ){
            ...
            if (CONTINUE == true){
                ...
                if (ACTION == "UR HandE - GripperOpen"){
                    ...
                    feedback_msg = "in the if";
                    goal_handle->publish_feedback(feedback);
                    //usleep(200000);
                    GripperCloseURRobotiq();
                    feedback_msg = "after calling Close";
                    goal_handle->publish_feedback(feedback);
                    //usleep(500000);
                    feedback_msg = "{STEP " + std::to_string(i) + "}: " + ACTION + ":Gripper closed successfully.";
                    goal_handle->publish_feedback(feedback);
...
int main(int argc, char ** argv)
{
    ...
    if (param_ROB == "ur10e" && param_EE != "none" && param_ENV == "bringup"){
        RCLCPP_INFO(logger, "before creating node");
