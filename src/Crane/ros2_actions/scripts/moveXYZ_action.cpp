...
private:
    ...
    const double INNER_RADIUS = 6.0;
    const double OUTER_RADIUS = 36.0;
    const double Z_MIN = 0.0;    // Ground level
    const double Z_MAX = 30.0;   // Maximum lift height for the crane

    bool check_boundaries(double x, double y, double z) {
        // Calculate the radial distance from the origin to the (x, y) point
        double radial_distance = sqrt(x * x + y * y);

        // Check if (x, y) position is within the annular workspace
        if (radial_distance < INNER_RADIUS || radial_distance > OUTER_RADIUS) {
            RCLCPP_ERROR(this->get_logger(), "The (x, y) position is out of bounds. It must be within the annular workspace: inner radius %.2f to outer radius %.2f", INNER_RADIUS, OUTER_RADIUS);
            return false;
        }

        // Check Z boundary as before
        if (z < Z_MIN || z > Z_MAX) {
            RCLCPP_ERROR(this->get_logger(), "The value in z-axis is not in the boundary of %.2f to %.2f", Z_MIN, Z_MAX);
            return false;
        }
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
        ...
        // Check if the target coordinates are within the specified boundaries
        if (!check_boundaries(positionX, positionY, positionZ)) {
            auto result = std::make_shared<MoveXYZ::Result>();
            result->result = "MoveXYZ:FAILED_OUT_OF_BOUNDS";
            goal_handle->abort(result);
            RCLCPP_ERROR(this->get_logger(), "Goal is out of bounds and cannot be executed.");
            return;
        }
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ///////////////////// This part is new for handeling the combination of rotational joint and prismatic joints //////////////
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // Manually calculate joint values for prismatic and rotational joints based on target XYZ
        std::vector<double> joint_group_positions;
        joint_group_positions.resize(8); // assuming 7 joints (1 rotational + 6 prismatic in chain)

        // 1. Calculate the required angle for the rotational joint to align with X-Y position.
        joint_group_positions[0] = atan2(positionY, positionX);  // Set the rotational joint (jib angle)
        joint_group_positions[7] = -atan2(positionY, positionX);

        // 2. Calculate the extension required for the first prismatic joint (trolley position).
        // This is the distance from the crane's origin to the target (X, Y) position
        double distance_xy = sqrt(positionX * positionX + positionY * positionY);
        joint_group_positions[1] = distance_xy - 6;  // First prismatic joint (trolley extension)

        // 3. Set the remaining prismatic joints to achieve the desired Z position.
        // Distribute the Z position equally or adjust based on your specific setup
        double z_extension_per_joint = (25 - positionZ) / 5.0; // Assuming 5 prismatic joints in the lifting chain
        for (int i = 2; i <= 6; ++i) {
            joint_group_positions[i] = z_extension_per_joint;  // Set prismatic joints for Z-axis lift
        }

        // Set the joint targets directly
